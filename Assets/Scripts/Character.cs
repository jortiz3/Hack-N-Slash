...
public abstract class Character : MonoBehaviour {
	...
	protected bool hpSliderIsPersistent;
	...
	protected virtual void Initialize() {
		...
		if (hpSlider != null) { //initialize hpslider values
	...
	protected internal void ReceiveDamage(float srcDmgVal, bool criticalHit) {
		...
		if (!isInvulnerable) { //enemies typically never invulnerable
			...
			if (gameObject.tag.Equals("Player")) { //player takes more damage based on difficulty
				...
				damage = srcDmgVal; //no damage bonus
			}

			if (damage < 1) { //minimum damage
				damage = 1;
			}
			...
			hpSlider.value = hp; //slider's max value should be set to maxhp
	...
	protected void UpdateAnimations() {
		...
		} else if (sr.color.Equals(Color.red)){ //flinching has ended && color still red 
			if (hp > 0) {
