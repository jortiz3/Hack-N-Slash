...
public class AdvertisementManager : MonoBehaviour, IStoreListener {
    private static bool optIn;
    private static bool bool_iap_purchased_NoAds;
    private static int optInCurrencyAmount = 15;
    private static int defaultCurrencyAmount = 5;
    private const string id_product_noads = "premium_noads";

    private IStoreController storeController;
    private Text text_iap_feedback;
    private GameObject button_iap_NoAds;
    private GameObject panel_iap_purchased_NoAds;
    private GameObject panel_iap_feedback;

    public static bool IAP_NoAds_Purchased { get { return bool_iap_purchased_NoAds; } }
    ...
    public void OnInitialized(IStoreController controller, IExtensionProvider provider) {
        Product product_NoAds = controller.products.WithID(id_product_noads); //get the noads product
        if (product_NoAds.hasReceipt) { //if it was purchased already
            PurchaseProduct(product_NoAds); //mark the product as purchased
        } else { //otherwise
            bool_iap_purchased_NoAds = false; //mark it as false
            panel_iap_purchased_NoAds.SetActive(false); //hide purchased panel
        }
    ...
    public void OnPurchaseFailed(Product item, PurchaseFailureReason r) { //in case the purchase fails -- called by IAP Button
        ...
        if (item.definition.id.Equals(id_product_noads)) { //if it is the right product
            switch (r) {
    ...
    public void PurchaseProduct(Product product) { //for codeless IAP
        switch (product.definition.id) {
            case id_product_noads:
                bool_iap_purchased_NoAds = true; //ensure gameManager knows it was purchased
                button_iap_NoAds.SetActive(false); //hide the button so they are unable to purchase again
                panel_iap_purchased_NoAds.SetActive(true); //show the purchased panel
    ...
    void Start() {
        ...
        button_iap_NoAds = GameObject.Find("Remove Ads Button"); //button to initiate purchase for noads
        panel_iap_purchased_NoAds = GameObject.Find("Remove Ads Purchased Panel"); //panel to display the player already purchased noads
        ...
        cb.AddProduct(id_product_noads, ProductType.NonConsumable); //add the noads product
